...
class Settings(BaseSettings):
    ...
    # SIP Trunk Settings
    sip_server_host: str = Field(default="0.0.0.0", env="SIP_SERVER_HOST")
    sip_server_port: int = Field(default=5060, env="SIP_SERVER_PORT")
    sip_server_domain: Optional[str] = Field(default=None, env="SIP_SERVER_DOMAIN")
    sip_enable_registration: bool = Field(default=True, env="SIP_ENABLE_REGISTRATION")
    sip_registration_timeout: int = Field(default=3600, env="SIP_REGISTRATION_TIMEOUT")  # 1 hour
    sip_max_concurrent_calls: int = Field(default=100, env="SIP_MAX_CONCURRENT_CALLS")
