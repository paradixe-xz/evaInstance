...
async def create_ollama_model(
    ...
    try:
        ...
        return {
            ...
            "modelfile_content": result.get("modelfile_content"),
...
async def chat_with_ollama_model(
    ...
    try:
        ...
        user_message = chat_request.message or request.query_params.get("message")
        if not user_message:
...
@router.post("/ollama/{agent_id}/chat/stream")
async def chat_with_ollama_model_stream(
    agent_id: int,
    chat_request: ChatRequest,
    request: Request,
    db: Session = Depends(get_db),
    ...
    """Chat streaming con modelo Ollama (NDJSON)."""
    try:
        agent_service = AgentService(db)
        agent = agent_service.get_agent_by_id(agent_id)
        ...
        if not agent.is_ollama_model:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Agent is not an Ollama model")
        ...
        def ndjson_generator():
            yield from agent_service.stream_chat_with_ollama_model(
                agent_id=agent_id,
                message=user_message,
                conversation_history=history
            )
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to stream chat: {str(e)}")